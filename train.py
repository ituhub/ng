...
def train_DQN(df, scaler, ticker):
    ...
    model_filename = f'dqn_model_{ticker}'
...
def train_cnn_lstm_attention(df, ticker, epochs=3):
    ...
    save_name = f"{ticker}_cnn_lstm_attention_model_tuned.h5"
    model.save(save_name)
    print(f"[{ticker}] CNN+LSTM+Attention model saved to {save_name}")
...
if __name__ == "__main__":
    tickers = [
        "CC=F", "GC=F", "KC=F", "NG=F", "^GDAXI", "^HSI", "USD/JPY", "ETHUSD", "SOLUSD", "^SPX", "HG=F", "SI=F", "CL=F", "^VIX", "ACB", "CGC", "CL=F", "TLRY"
    ]
    ...
    for ticker in tickers:
        ...
        if ode_func is not None:
            torch.save(ode_func.state_dict(), "neural_ode_func.pth")
