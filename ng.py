...
def load_hmm_model():
    if not os.path.exists("hmm_model.pkl"):
        st.warning("No HMM model found at hmm_model.pkl")
        return None
    return joblib.load("hmm_model.pkl")
...
def load_neural_ode_func():
    if not os.path.exists("neural_ode_func.pth"):
        st.warning("No Neural ODE function at neural_ode_func.pth")
        return None
    func = ODEFunc(hidden_dim=16)
    func.load_state_dict(torch.load("neural_ode_func.pth", map_location=torch.device('cpu')))
    func.eval()
    return func
...
def display_cnn_predictions(df, df_preds, ticker):
    ...
    st.subheader("📈 Dummy 1-Day Forecast Range")
    st.dataframe(pd.DataFrame({"Date": future_dates, "Predicted_Price": future_prices}))

###############################################################################
# 10) Main Streamlit
###############################################################################
st.title("Financial Predictions (No Transformer)")
